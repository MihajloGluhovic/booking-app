...
export class RoomService {
  ...
  getRooms(): Observable<RoomInterface[]> {
    ...
    return this.http.get<RoomInterface[]>(fullUrl, { headers }).pipe(
      tap((rooms) => this.roomsSubject.next(rooms)) // Updates roomsSubject with new data
    );
  }

  getRoomsOnDate(date: string): Observable<RoomInterface[]> {
    this.isLoadingSubject.next(true);
    const fullUrl = environment.apiUrl + date;
    return this.http.get<RoomInterface[]>(fullUrl).pipe(
      tap((rooms) => {
  ...
  refreshRooms(): void {
    this.getRooms().subscribe(); // Triggers an update to rooms$
  }
}
